...
select week('2022-10-30', 1);              -- 모드값을 따로 지정 가능. (mode는 주의 시작을 일요일로 볼지~ 월요일로 볼지에 따라 다양함) weekofyear과 비슷
...
-- 주문한 년도별 가격과 평균가격
select year(orderdate) '년도', ceiling(sum(saleprice)/count(saleprice)) '평균'
from orders
...
create or replace view v_weekly(weekly, date, min, max)                -- 이름, 순서 동일하도록 컬럼 지정!
as select yearweek(orderdate) 'weekly', 
          orderdate 'date', 
          min(saleprice) 'min', 
          max(saleprice) 'max' 
from orders
...
select
case dayofweek(orderdate)                 -- if 느낌..? 뭔지는 정확히 설명 안 해줌.
     when 1 then '일'
     when 2 then '월'
     when 3 then '화'
     when 4 then '수'
