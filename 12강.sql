...
-- minus 대체, not in
-- 대한민국에 거주하는 고객 중 도서를 주문한 고객의 이름을 제외하고 출력.
...
-- 대한민국에 거주하는 고객 중 도서를 주문한 고객의 이름을 출력.
select username from customer
where address like '%대한민국%' 
...
where exists (select custid from orders o where c.custid = o.custid);





-- 주문일자가 2021-02-05 이후 주문 금액보다 가격이 비싼 모든 도서를 출력하시오.
-- all (서브쿼리의 결과와 메인쿼리의 비교를 모두 비교. 합집합 느낌~)
select * from book
where price > all (select saleprice from orders where orderdate > '2021-02-05');
