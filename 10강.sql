...
select avg(o.saleprice) '평균주문가격' , avg(b.price) '평균가격' from orders o, book b;     -- as 생략 가능.        

-- 총주문량과 평균판매가
select count(*) '총주문량' , avg(saleprice) '평균판매가' from orders;            
select count(*) '총주문량' , 
avg(ifnull(saleprice,0)) '평균판매가' from orders;                             -- 만약 null이 있으면-> 오류나니까 안전하게 ifnull로 대체값(0) 지정. 
...
select sum(saleprice) '총매출',
       ...
       min(saleprice) '최저가',
       max(saleprice) '최고가',
       count(distinct b.bookid) '판매 도서수'                                  -- 중복 배제
from orders o, book b                                                       -- as와 ''생략하고 별칭줄 수 있음. 
...
select custid,
       count(*) '총 수량',
       sum(saleprice) '총 판매액'
from orders group by custid;                    -- group by 할 때, 그룹핑한 컬럼과 select하려는 컬럼이 동일한지 확인! select 뒤에 *여도 괜츈~


-- 고객이 주문한 도서의 총 판매액, 평균값, 최저가, 최고가와 고객아이디를 출력하시오.
select custid,
       sum(saleprice) '총 판매액',
       avg(saleprice) '평균 구매가',
       min(saleprice) '최저가',
       max(saleprice) '최고가'
...
select o.custid as '고객아이디',
       c.username as '고객명',
       count(o.saleprice) '주문수량',
       sum(o.saleprice) '판매금액'
from orders o, customer c 
where o.saleprice >=8000 and c.custid = o.custid                   
group by o.custid
