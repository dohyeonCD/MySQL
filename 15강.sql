...
select username '고객명',
       ...
       format(avg(saleprice),1) '평균',                         -- format (x,d) x형식의 문자열~을 d자리수로 맞춘다. 날짜 형식으로도 가능.
       min(saleprice) '최소 구매액', max(saleprice) '최대 구매액'
from customer c
...
select format(sum(saleprice),0) '합계',
	   format(avg(saleprice),1) '평균',
       min(saleprice) '최소', max(saleprice) '최대',
...
-- 2단계: 
-- 모든 가격을 결합한다.
select group_concat(saleprice order by saleprice separator '..' )       -- 한 컬럼을 가지고 결합할 때는 이렇게. 정렬을 먼저 시키고 구분하기~
from orders;
...
select min(saleprice) 'min',
       substring_index
       (substring_index(group_concat(saleprice order by saleprice separator ','), ',',25/100 * count(saleprice))
       ,',',-1) '25%',                        -- -1을 한 이유는 최소의 최대값을 구하려고.
       substring_index
       (substring_index(group_concat(saleprice order by saleprice separator ','), ',',50/100 * count(saleprice))
       ,',',-1) '50%',
       format(avg(saleprice),1) '평균',
       substring_index
       (substring_index(group_concat(saleprice order by saleprice separator ','), ',',75/100 * count(saleprice))
       ,',',-1) '75%',
       max(saleprice) 'max'
from orders;
...
select min(saleprice) 'min',
       format(25/100 * sum(saleprice),1) '25%',
       format(50/100 * sum(saleprice),1) '50%',
       format(avg(saleprice),1) 'mean',
       format(75/100 * sum(saleprice),1) '75%',
       sum(saleprice) 'sum'
from orders;
